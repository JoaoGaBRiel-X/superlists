...
class NewVisitorTest(StaticLiveServerTestCase):
	...
	def test_can_start_a_list_and_retrieve_it_later(self):
		...
		# Quando tecla enter, a página é atualizada, e agora a página lista
		# "1: Buy peacock feathers" como um item em uma lista de tarefas
		inputbox.send_keys(Keys.ENTER)
		self.wait_for_row_in_list_table('1: Buy peacock feathers')

		# Ainda continua havendo uma caixa de texto convidando-a a acrescentar outro
		# item. Ela insere "Use peacock feathers to make fly" (Usar penas de pavão
		# para fazer um flu - Edith é bem metódica)
		inputbox = self.browser.find_element_by_id('id_new_item')
	...
	def test_multiple_users_can_start_lists_at_different_urls(self):
		...
		# Novamente não há nenhum sinal da lista Edith
		page_text = self.browser.find_element_by_tag_name('body').text
		self.assertNotIn('Buy peacock feathers', page_text)
